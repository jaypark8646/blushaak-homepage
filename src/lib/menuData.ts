import { MenuItem, MenuCategoryInfo } from "@/types";

export const MENU_CATEGORIES: MenuCategoryInfo[] = [
  { id: "new", label: "New", labelKo: "신상" },
  { id: "coffee", label: "Coffee", labelKo: "커피" },
  { id: "beverage", label: "Beverage", labelKo: "음료" },
  { id: "blended", label: "Blended", labelKo: "블렌디드" },
  { id: "bakery", label: "Bakery", labelKo: "베이커리" },
  { id: "ice-cream", label: "Ice-cream", labelKo: "아이스크림" },
  { id: "md", label: "MD", labelKo: "MD 상품" },
  { id: "deli", label: "Deli", labelKo: "델리" },
];

export const MENU_ITEMS: MenuItem[] = [
  // New
  {
    id: "new-a2-milk-deep-latte",
    name: "A2 Milk Deep Latte",
    nameKo: "A2우유 딥 라떼",
    category: "new",
    image: "/images/menu/new-a2-milk-deep-latte.jpg",
    nutrition: null,
    isNew: true,
  },
  {
    id: "new-red-bean-bingsu",
    name: "Red Bean Cup Bingsu",
    nameKo: "레드빈 컵빙수",
    category: "new",
    image: "/images/menu/new-red-bean-bingsu.jpg",
    nutrition: null,
    isNew: true,
  },
  {
    id: "new-mango-bingsu",
    name: "Mango Cup Bingsu",
    nameKo: "망고 컵빙수",
    category: "new",
    image: "/images/menu/new-mango-bingsu.jpg",
    nutrition: null,
    isNew: true,
  },
  {
    id: "new-wildflower-honey-tomato-juice",
    name: "Peung Wildflower Honey Tomato Juice",
    nameKo: "프응 야생화 꿀 토마토 주스",
    category: "new",
    image: "/images/menu/new-wildflower-honey-tomato-juice.jpg",
    nutrition: null,
    isNew: true,
  },
  {
    id: "new-wildflower-honey-chamoe-juice",
    name: "Peung Wildflower Honey Chamoe Juice",
    nameKo: "프응 야생화 꿀 참외 주스",
    category: "new",
    image: "/images/menu/new-wildflower-honey-chamoe-juice.jpg",
    nutrition: null,
    isNew: true,
  },
  {
    id: "new-wildflower-honey-matcha-lemon-tea",
    name: "Peung Wildflower Honey Matcha Lemon Tea",
    nameKo: "프응 야생화 꿀 말차 레몬 티",
    category: "new",
    image: "/images/menu/new-wildflower-honey-matcha-lemon-tea.jpg",
    nutrition: null,
    isNew: true,
  },
  // Coffee
  {
    id: "shaak-latte",
    name: "Shaak Latte",
    nameKo: "샥라떼",
    category: "coffee",
    image: "/images/menu/shaak-latte.jpg",
    nutrition: { calories: 150.3, sodium: 78.2, sugar: 14.1, saturatedFat: 5.6, protein: 4.6, caffeine: 85.1, allergens: "우유/대두" },
  },
  {
    id: "peanut-latte",
    name: "Peanut Latte",
    nameKo: "피넛라떼",
    category: "coffee",
    image: "/images/menu/peanut-latte.jpg",
    nutrition: { calories: 233.7, sodium: 105.2, sugar: 14.3, saturatedFat: 6.4, protein: 6.4, caffeine: 112.8, allergens: "우유/대두/밀" },
  },
  {
    id: "night-americano",
    name: "Night Americano",
    nameKo: "나이트 아메리카노",
    category: "coffee",
    image: "/images/menu/americano.jpg",
    nutrition: { calories: 12.0, sodium: 15.0, sugar: 0.0, saturatedFat: 0.0, protein: 1.0, caffeine: 216.6, allergens: null },
  },
  {
    id: "colombian-fruity-americano",
    name: "Colombian Fruity Americano",
    nameKo: "콜롬비안 프루티 아메리카노",
    category: "coffee",
    image: "/images/menu/americano.jpg",
    nutrition: null,
  },
  {
    id: "decaf-americano",
    name: "Decaf Americano",
    nameKo: "디카페인 아메리카노",
    category: "coffee",
    image: "/images/menu/americano.jpg",
    nutrition: { calories: 12.0, sodium: 5.0, sugar: 0.0, saturatedFat: 0.0, protein: 0.0, caffeine: 2.29, allergens: null },
  },
  {
    id: "white-americano",
    name: "White Americano",
    nameKo: "화이트 아메리카노",
    category: "coffee",
    image: "/images/menu/white-americano.jpg",
    nutrition: { calories: 114.2, sodium: 40.0, sugar: 5.7, saturatedFat: 4.4, protein: 1.3, caffeine: 188.7, allergens: "우유" },
  },
  {
    id: "cafe-latte",
    name: "Cafe Latte",
    nameKo: "카페 라떼",
    category: "coffee",
    image: "/images/menu/cafe-latte.jpg",
    nutrition: { calories: 143.0, sodium: 116.3, sugar: 10.7, saturatedFat: 5.3, protein: 8.0, caffeine: 99.6, allergens: "우유" },
  },
  {
    id: "vanilla-bean-latte",
    name: "Vanilla Bean Latte",
    nameKo: "바닐라 빈 라떼",
    category: "coffee",
    image: "/images/menu/vanilla-bean-latte.jpg",
    nutrition: { calories: 198.9, sodium: 103.9, sugar: 28.0, saturatedFat: 4.4, protein: 7.1, caffeine: 102.7, allergens: "우유" },
  },
  {
    id: "zero-vanilla-latte",
    name: "Zero Vanilla Latte",
    nameKo: "제로 바닐라 라떼",
    category: "coffee",
    image: "/images/menu/zero-vanilla-latte.jpg",
    nutrition: { calories: 201.6, sodium: 74.2, sugar: 10.7, saturatedFat: 3.6, protein: 5.8, caffeine: 199.8, allergens: "우유" },
  },
  {
    id: "dolce-latte",
    name: "Dolce Latte",
    nameKo: "돌체 라떼",
    category: "coffee",
    image: "/images/menu/dolce-latte.jpg",
    nutrition: { calories: 252.2, sodium: 151.8, sugar: 30.7, saturatedFat: 5.8, protein: 8.0, caffeine: 110.4, allergens: "우유/대두" },
  },
  {
    id: "jeju-matcha-cafe-latte",
    name: "Matcha Cafe Latte",
    nameKo: "말차 카페 라떼",
    category: "coffee",
    image: "/images/menu/jeju-matcha-cafe-latte.jpg",
    nutrition: { calories: 348.9, sodium: 127.9, sugar: 32.9, saturatedFat: 8.4, protein: 13.3, caffeine: 383.6, allergens: "우유/대두" },
  },
  {
    id: "caramel-macchiato",
    name: "Caramel Macchiato",
    nameKo: "카라멜 마끼야또",
    category: "coffee",
    image: "/images/menu/caramel-macchiato.jpg",
    nutrition: { calories: 242.0, sodium: 113.7, sugar: 17.8, saturatedFat: 4.9, protein: 7.1, caffeine: 91.8, allergens: "우유/대두" },
  },
  {
    id: "cafe-mocha",
    name: "Cafe Mocha",
    nameKo: "카페 모카",
    category: "coffee",
    image: "/images/menu/cafe-mocha.jpg",
    nutrition: { calories: 247.7, sodium: 117.3, sugar: 29.9, saturatedFat: 3.6, protein: 7.2, caffeine: 222.0, allergens: "우유/대두" },
  },
  {
    id: "cold-brew-americano",
    name: "Cold Brew Americano",
    nameKo: "콜드브루 아메리카노",
    category: "coffee",
    image: "/images/menu/cold-brew-americano.jpg",
    nutrition: { calories: 5.7, sodium: 2.4, sugar: 0.0, saturatedFat: 0.0, protein: 0.2, caffeine: 142.5, allergens: null },
  },
  {
    id: "cold-brew-latte",
    name: "Cold Brew Latte",
    nameKo: "콜드브루 라떼",
    category: "coffee",
    image: "/images/menu/cold-brew-latte.jpg",
    nutrition: { calories: 98.8, sodium: 74.2, sugar: 7.2, saturatedFat: 3.9, protein: 4.5, caffeine: 142.8, allergens: "우유" },
  },
  {
    id: "banana-cold-brew-oat-latte",
    name: "Banana Cold Brew Oat Latte",
    nameKo: "바나나 콜드브루 오트라떼",
    category: "coffee",
    image: "/images/menu/banana-cold-brew-oat-latte.jpg",
    nutrition: { calories: 203.3, sodium: 94.6, sugar: 23.7, saturatedFat: 0.0, protein: 0.0, caffeine: 86.6, allergens: null },
  },
  {
    id: "dolce-cold-brew-latte",
    name: "Dolce Cold Brew Latte",
    nameKo: "돌체 콜드브루 라떼",
    category: "coffee",
    image: "/images/menu/dolce-cold-brew-latte.jpg",
    nutrition: { calories: 183.5, sodium: 69.5, sugar: 22.0, saturatedFat: 3.6, protein: 3.9, caffeine: 134.1, allergens: "우유/대두" },
  },
  {
    id: "ah-shot-chu",
    name: "Peach Iced Tea With Espresso",
    nameKo: "아샷추",
    category: "coffee",
    image: "/images/menu/ah-shot-chu.jpg",
    nutrition: { calories: 172.3, sodium: 12.4, sugar: 38.2, saturatedFat: 0.0, protein: 0.4, caffeine: 113.2, allergens: "복숭아/아황산류" },
  },
  {
    id: "cinnamon-cafe-latte",
    name: "Cinnamon Cafe Latte",
    nameKo: "시나몬 카페 라떼",
    category: "coffee",
    image: "/images/menu/cinnamon-cafe-latte.jpg",
    nutrition: { calories: 197.1, sodium: 76.4, sugar: 27.5, saturatedFat: 2.2, protein: 6.2, caffeine: 136.3, allergens: "우유" },
  },
  {
    id: "hazelnut-oat-latte",
    name: "Hazlenut Oat Latte",
    nameKo: "헤이즐넛 오트라떼",
    category: "coffee",
    image: "/images/menu/hazelnut-oat-latte.jpg",
    nutrition: null,
  },
  {
    id: "cafe-citrus",
    name: "Cafe Citrus",
    nameKo: "카페 시트러스",
    category: "coffee",
    image: "/images/menu/cafe-citrus.jpg",
    nutrition: { calories: 100.8, sodium: 29.4, sugar: 19.0, saturatedFat: 0.1, protein: 1.3, caffeine: 125.8, allergens: null },
  },
  {
    id: "cafe-pieno",
    name: "Cafe Pieno",
    nameKo: "카페 피에노",
    category: "coffee",
    image: "/images/menu/cafe-pieno.jpg",
    nutrition: { calories: 164.3, sodium: 28.2, sugar: 9.9, saturatedFat: 7.3, protein: 1.5, caffeine: 137.8, allergens: "우유/대두" },
  },
  {
    id: "espresso",
    name: "Espresso",
    nameKo: "에스프레소",
    category: "coffee",
    image: "/images/menu/espresso.jpg",
    nutrition: { calories: 2.5, sodium: 6.1, sugar: 0.0, saturatedFat: 0.0, protein: 0.2, caffeine: 124.0, allergens: null },
  },
  // Beverage
  {
    id: "golden-yuzu-beauty-elixir",
    name: "Golden Yuzu Beauty Elixir",
    nameKo: "골든유자 뷰티 엘릭서",
    category: "beverage",
    image: "/images/menu/golden-yuzu-beauty-elixir.jpg",
    nutrition: { calories: 180.9, sodium: 12.9, sugar: 34.5, saturatedFat: 0.0, protein: 0.1, caffeine: null, allergens: null },
  },
  {
    id: "apple-mango-iced-tea",
    name: "Apple Mango Iced Tea",
    nameKo: "애플망고 아이스 티",
    category: "beverage",
    image: "/images/menu/apple-mango-iced-tea.jpg",
    nutrition: { calories: 184.7, sodium: 35.5, sugar: 44.4, saturatedFat: 0.0, protein: 0.4, caffeine: null, allergens: "복숭아/아황산류" },
  },
  {
    id: "lemon-ade",
    name: "Lemon Ade",
    nameKo: "레몬 에이드",
    category: "beverage",
    image: "/images/menu/lemon-ade.jpg",
    nutrition: { calories: 113.7, sodium: 11.5, sugar: 26.6, saturatedFat: 0.0, protein: 0.0, caffeine: null, allergens: null },
  },
  {
    id: "grapefruit-ade",
    name: "Grapefruit Ade",
    nameKo: "자몽 에이드",
    category: "beverage",
    image: "/images/menu/grapefruit-ade.jpg",
    nutrition: { calories: 175.7, sodium: 18.2, sugar: 37.7, saturatedFat: 0.0, protein: 0.8, caffeine: null, allergens: null },
  },
  {
    id: "jeju-matcha-latte",
    name: "Matcha Latte",
    nameKo: "말차 라떼",
    category: "beverage",
    image: "/images/menu/jeju-matcha-latte.jpg",
    nutrition: { calories: 433.4, sodium: 148.3, sugar: 46.6, saturatedFat: 8.4, protein: 15.1, caffeine: 180.7, allergens: "우유/대두" },
  },
  {
    id: "seolhyang-strawberry-latte",
    name: "Seolhyang Strawberry Latte",
    nameKo: "설향 딸기 라떼",
    category: "beverage",
    image: "/images/menu/seolhyang-strawberry-latte.jpg",
    nutrition: { calories: 202.0, sodium: 89.2, sugar: 29.3, saturatedFat: 3.6, protein: 4.4, caffeine: null, allergens: "우유" },
  },
  {
    id: "jeju-misutgaru",
    name: "Barley Misutgaru",
    nameKo: "보리 미숫가루",
    category: "beverage",
    image: "/images/menu/jeju-misutgaru.jpg",
    nutrition: { calories: 277.1, sodium: 72.8, sugar: 28.9, saturatedFat: 2.7, protein: 8.0, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "beverage-valrhona-chocolate",
    name: "Valrhona Chocolate",
    nameKo: "발로나 초콜릿",
    category: "beverage",
    image: "/images/menu/valrhona-chocolate-20260330.jpg",
    nutrition: null,
  },
  {
    id: "honey-grapefruit-tea",
    name: "Honey Grapefruit Tea",
    nameKo: "꿀 자몽 티",
    category: "beverage",
    image: "/images/menu/honey-grapefruit-tea.jpg",
    nutrition: { calories: 186.0, sodium: 11.1, sugar: 42.6, saturatedFat: 0.0, protein: 0.4, caffeine: null, allergens: null },
  },
  {
    id: "lemon-yuzu-tea",
    name: "Lemon Yuzu Tea",
    nameKo: "레몬 유자 티",
    category: "beverage",
    image: "/images/menu/lemon-yuzu-tea.jpg",
    nutrition: { calories: 222.4, sodium: 48.8, sugar: 51.9, saturatedFat: 0.0, protein: 0.0, caffeine: null, allergens: null },
  },
  {
    id: "royal-milk-tea",
    name: "Royal Milk Tea",
    nameKo: "로얄 밀크 티",
    category: "beverage",
    image: "/images/menu/royal-milk-tea.jpg",
    nutrition: { calories: 212.7, sodium: 97.7, sugar: 29.8, saturatedFat: 1.8, protein: 5.8, caffeine: 18.9, allergens: "우유" },
  },
  {
    id: "lemon-earl-grey-tea",
    name: "Lemon Earl Grey Tea",
    nameKo: "레몬 얼그레이 티",
    category: "beverage",
    image: "/images/menu/lemon-earl-grey-tea.jpg",
    nutrition: { calories: 135.0, sodium: 5.8, sugar: 5.5, saturatedFat: 0.0, protein: 0.9, caffeine: 30.9, allergens: null },
  },
  {
    id: "mango-latte",
    name: "Mango Latte",
    nameKo: "망고 라떼",
    category: "beverage",
    image: "/images/menu/mango-latte.jpg",
    nutrition: { calories: 239.8, sodium: 89.2, sugar: 40.0, saturatedFat: 4.0, protein: 5.3, caffeine: null, allergens: "우유/복숭아" },
  },
  {
    id: "green-grape-ade",
    name: "Green Grape Ade",
    nameKo: "청포도 에이드",
    category: "beverage",
    image: "/images/menu/green-grape-ade.jpg",
    nutrition: { calories: 163.4, sodium: 28.0, sugar: 27.1, saturatedFat: 0.0, protein: 0.4, caffeine: null, allergens: null },
  },
  {
    id: "yeongam-sweet-potato-latte",
    name: "Yeongam Sweet Potato Latte",
    nameKo: "영암 고구마 라떼",
    category: "beverage",
    image: "/images/menu/yeongam-sweet-potato-latte.jpg",
    nutrition: { calories: 259.7, sodium: 72.8, sugar: 30.2, saturatedFat: 3.6, protein: 5.8, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "honey-chamomile",
    name: "Honey Chamomile",
    nameKo: "허니 캐모마일",
    category: "beverage",
    image: "/images/menu/honey-chamomile.jpg",
    nutrition: { calories: 3.6, sodium: 16.0, sugar: 0.0, saturatedFat: 0.0, protein: 0.0, caffeine: null, allergens: null },
  },
  {
    id: "berry-hibiscus",
    name: "Berry Hibiscus",
    nameKo: "베리 히비스커스",
    category: "beverage",
    image: "/images/menu/berry-hibiscus.jpg",
    nutrition: { calories: 9.8, sodium: 11.5, sugar: 0.0, saturatedFat: 0.0, protein: 0.0, caffeine: 0.4, allergens: null },
  },
  {
    id: "ginger-pear-tea",
    name: "Ginger Pear Tea",
    nameKo: "진저 배 티",
    category: "beverage",
    image: "/images/menu/ginger-pear-tea.jpg",
    nutrition: { calories: 145.6, sodium: 52.8, sugar: 34.6, saturatedFat: 0.0, protein: 0.4, caffeine: null, allergens: null },
  },
  {
    id: "pure-milk",
    name: "Pure Milk",
    nameKo: "순수 우유",
    category: "beverage",
    image: "/images/menu/pure-milk.jpg",
    nutrition: { calories: 149.5, sodium: 115.0, sugar: 11.5, saturatedFat: 6.2, protein: 6.9, caffeine: null, allergens: "우유" },
  },
  {
    id: "strawberry-iced-tea",
    name: "Strawberry Iced Tea",
    nameKo: "딸기 아이스 티",
    category: "beverage",
    image: "/images/menu/strawberry-iced-tea.jpg",
    nutrition: { calories: 102.6, sodium: 6.7, sugar: 22.6, saturatedFat: 0.0, protein: 0.0, caffeine: null, allergens: null },
  },
  {
    id: "zero-peach-iced-tea",
    name: "Zero Peach Iced Tea",
    nameKo: "제로 복숭아 아이스티",
    category: "beverage",
    image: "/images/menu/zero-peach-iced-tea.jpg",
    nutrition: { calories: 14.0, sodium: 10.2, sugar: 0.4, saturatedFat: 0.0, protein: 0.0, caffeine: 15.1, allergens: "복숭아" },
  },
  {
    id: "double-choco-latte",
    name: "Double Choco Latte",
    nameKo: "더블 초코라떼",
    category: "beverage",
    image: "/images/menu/double-choco-latte.jpg",
    nutrition: { calories: 294.4, sodium: 145.6, sugar: 33.3, saturatedFat: 2.7, protein: 8.0, caffeine: 6.7, allergens: "우유/대두" },
  },
  {
    id: "beverage-toffee-nut-latte",
    name: "Toffee Nut Latte",
    nameKo: "토피넛 라떼",
    category: "beverage",
    image: "/images/menu/toffee-nut-latte-20260330.jpg",
    nutrition: null,
  },
  {
    id: "beverage-earl-grey-vanilla-tea-latte",
    name: "Earl Grey Vanilla Tea Latte",
    nameKo: "얼그레이 바닐라 티 라떼",
    category: "beverage",
    image: "/images/menu/earl-grey-vanilla-tea-latte-20260330.jpg",
    nutrition: null,
  },
  {
    id: "beverage-brown-sugar-bubble-tea",
    name: "Brown Sugar Bubble Tea",
    nameKo: "흑당 버블티",
    category: "beverage",
    image: "/images/menu/brown-sugar-bubble-tea-20260330.jpg",
    nutrition: null,
  },
  {
    id: "banana-bubble-tea",
    name: "Banana Bubble Tea",
    nameKo: "바나나 버블티",
    category: "beverage",
    image: "/images/menu/banana-bubble-tea.jpg",
    nutrition: { calories: 234.9, sodium: 76.4, sugar: 33.3, saturatedFat: 3.6, protein: 5.3, caffeine: null, allergens: null },
  },
  {
    id: "jeju-matcha-bubble-tea",
    name: "Matcha Bubble Tea",
    nameKo: "말차 버블티",
    category: "beverage",
    image: "/images/menu/jeju-matcha-bubble-tea.jpg",
    nutrition: { calories: 472.2, sodium: 140.3, sugar: 44.0, saturatedFat: 8.4, protein: 12.0, caffeine: 155.0, allergens: "우유/대두" },
  },
  {
    id: "double-choco-bubble-tea",
    name: "Double Choco Bubble Tea",
    nameKo: "더블 초코 버블티",
    category: "beverage",
    image: "/images/menu/double-choco-bubble-tea.jpg",
    nutrition: { calories: 296.6, sodium: 120.8, sugar: 31.1, saturatedFat: 5.3, protein: 7.1, caffeine: 5.8, allergens: "우유/대두" },
  },
  {
    id: "beverage-red-apple-yuja-tea",
    name: "Red Apple Yuja Tea",
    nameKo: "홍사과 유자티",
    category: "beverage",
    image: "/images/menu/red-apple-yuja-tea-20260330.jpg",
    nutrition: null,
  },
  // Blended
  {
    id: "strawberry-yogurt-smoothie",
    name: "Strawberry Yogurt Smoothie",
    nameKo: "딸기 요거트 스무디",
    category: "blended",
    image: "/images/menu/strawberry-yogurt-smoothie.jpg",
    nutrition: { calories: 351.2, sodium: 68.4, sugar: 65.3, saturatedFat: 1.8, protein: 5.8, caffeine: null, allergens: "우유" },
  },
  {
    id: "plain-yogurt-smoothie",
    name: "Plain Yogurt Smoothie",
    nameKo: "플레인 요거트 스무디",
    category: "blended",
    image: "/images/menu/plain-yogurt-smoothie.jpg",
    nutrition: { calories: 392.5, sodium: 83.0, sugar: 65.7, saturatedFat: 0.9, protein: 5.8, caffeine: null, allergens: "우유" },
  },
  {
    id: "blueberry-yogurt-smoothie",
    name: "Blueberry Yogurt Smoothie",
    nameKo: "블루베리 요거트 스무디",
    category: "blended",
    image: "/images/menu/blueberry-yogurt-smoothie.jpg",
    nutrition: { calories: 389.8, sodium: 66.2, sugar: 63.9, saturatedFat: 1.8, protein: 5.3, caffeine: null, allergens: "우유" },
  },
  {
    id: "mango-banana-smoothie",
    name: "Mango Banana Smoothie",
    nameKo: "망고 바나나 스무디",
    category: "blended",
    image: "/images/menu/mango-banana-smoothie.jpg",
    nutrition: { calories: 220.2, sodium: 22.6, sugar: 51.1, saturatedFat: 0.0, protein: 0.9, caffeine: null, allergens: null },
  },
  {
    id: "jeju-matcha-frappe",
    name: "Matcha Frappe",
    nameKo: "말차 프라페",
    category: "blended",
    image: "/images/menu/jeju-matcha-frappe.jpg",
    nutrition: { calories: 766.9, sodium: 186.9, sugar: 116.3, saturatedFat: 12.9, protein: 14.2, caffeine: 217.6, allergens: "우유/대두" },
  },
  {
    id: "mint-choco-frappe",
    name: "Mint Choco Frappe",
    nameKo: "민트 초코 프라페",
    category: "blended",
    image: "/images/menu/mint-choco-frappe.jpg",
    nutrition: { calories: 352.1, sodium: 143.0, sugar: 59.9, saturatedFat: 3.6, protein: 6.7, caffeine: 25.3, allergens: "우유/대두" },
  },
  {
    id: "rice-shake",
    name: "Rice Shake",
    nameKo: "라이스 쉐이크",
    category: "blended",
    image: "/images/menu/rice-shake.jpg",
    nutrition: { calories: 485.8, sodium: 216.7, sugar: 33.3, saturatedFat: 4.9, protein: 4.4, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "milk-shake",
    name: "Milk Shake",
    nameKo: "밀크쉐이크",
    category: "blended",
    image: "/images/menu/milk-shake.jpg",
    nutrition: { calories: 486.6, sodium: 153.2, sugar: 59.9, saturatedFat: 12.0, protein: 3.1, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "strawberry-shake",
    name: "Strawberry Shake",
    nameKo: "딸기 쉐이크",
    category: "blended",
    image: "/images/menu/strawberry-shake.jpg",
    nutrition: { calories: 444.9, sodium: 213.6, sugar: 49.7, saturatedFat: 5.3, protein: 4.0, caffeine: null, allergens: "우유/대두/아황산류" },
  },
  {
    id: "honey-pear-smoothie",
    name: "Honey Pear Smoothie",
    nameKo: "꿀배 스무디",
    category: "blended",
    image: "/images/menu/honey-pear-smoothie.jpg",
    nutrition: { calories: 284.2, sodium: 87.0, sugar: 57.3, saturatedFat: 0.0, protein: 0.4, caffeine: null, allergens: null },
  },
  {
    id: "green-grape-smoothie",
    name: "Green Grape Smoothie",
    nameKo: "청포도 스무디",
    category: "blended",
    image: "/images/menu/green-grape-smoothie.jpg",
    nutrition: { calories: 312.6, sodium: 40.0, sugar: 72.8, saturatedFat: 0.0, protein: 0.4, caffeine: null, allergens: null },
  },
  {
    id: "red-grapefruit-smoothie",
    name: "Red Grapefruit Smoothie",
    nameKo: "레드자몽 스무디",
    category: "blended",
    image: "/images/menu/red-grapefruit-smoothie.jpg",
    nutrition: { calories: 262.9, sodium: 21.8, sugar: 62.2, saturatedFat: 0.0, protein: 0.9, caffeine: null, allergens: null },
  },
  {
    id: "coffee-shake",
    name: "Coffee Shake",
    nameKo: "커피 쉐이크",
    category: "blended",
    image: "/images/menu/coffee-shake.jpg",
    nutrition: { calories: 323.2, sodium: 103.0, sugar: 63.9, saturatedFat: 3.1, protein: 4.9, caffeine: 215.3, allergens: "우유/대두" },
  },
  {
    id: "strawberry-yuzu-smoothie",
    name: "Strawberry Yuzu Smoothie",
    nameKo: "딸기 유자 스무디",
    category: "blended",
    image: "/images/menu/strawberry-yuzu-smoothie.jpg",
    nutrition: { calories: 341.0, sodium: 52.4, sugar: 82.1, saturatedFat: 0.0, protein: 0.4, caffeine: null, allergens: null },
  },
  {
    id: "strawberry-banana-juice",
    name: "Strawberry Banana Juice",
    nameKo: "딸기 바나나 주스",
    category: "blended",
    image: "/images/menu/strawberry-banana-juice.jpg",
    nutrition: { calories: 302.8, sodium: 83.5, sugar: 51.1, saturatedFat: 4.0, protein: 7.5, caffeine: null, allergens: "우유" },
  },
  {
    id: "mango-yogurt-smoothie",
    name: "Mango Yogurt Smoothie",
    nameKo: "망고 요거트 스무디",
    category: "blended",
    image: "/images/menu/mango-yogurt-smoothie.jpg",
    nutrition: { calories: 349.0, sodium: 67.0, sugar: 62.6, saturatedFat: 1.3, protein: 4.4, caffeine: null, allergens: "우유" },
  },
  {
    id: "choco-shake",
    name: "Choco Shake",
    nameKo: "초코 쉐이크",
    category: "blended",
    image: "/images/menu/choco-shake.jpg",
    nutrition: { calories: 383.6, sodium: 182.9, sugar: 47.1, saturatedFat: 8.9, protein: 8.0, caffeine: 11.5, allergens: "우유/대두" },
  },
  {
    id: "choco-banana-smoothie",
    name: "Choco Banana Smoothie",
    nameKo: "초코 바나나 스무디",
    category: "blended",
    image: "/images/menu/choco-banana-smoothie.jpg",
    nutrition: { calories: 342.3, sodium: 129.2, sugar: 48.0, saturatedFat: 6.2, protein: 6.7, caffeine: 7.1, allergens: "우유/대두" },
  },
  {
    id: "cup-bingsu",
    name: "Cup Shaved Ice",
    nameKo: "컵빙수",
    category: "blended",
    image: "/images/menu/cup-bingsu.jpg",
    nutrition: { calories: 413.0, sodium: 139.0, sugar: 61.3, saturatedFat: 4.0, protein: 3.1, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "real-strawberry-juice",
    name: "Real Strawberry Juice",
    nameKo: "리얼 딸기 주스",
    category: "blended",
    image: "/images/menu/real-strawberry-juice-20260330.jpg",
    nutrition: null,
  },
  {
    id: "hadong-plum-smoothie",
    name: "Hadong Plum Smoothie",
    nameKo: "하동 매실 스무디",
    category: "blended",
    image: "/images/menu/hadong-plum-smoothie.jpg",
    nutrition: { calories: 289.5, sodium: 51.5, sugar: 59.5, saturatedFat: 0.0, protein: 1.8, caffeine: null, allergens: null },
  },
  {
    id: "jeju-lemon-smoothie",
    name: "Jeju Lemon Smoothie",
    nameKo: "제주 레몬 스무디",
    category: "blended",
    image: "/images/menu/jeju-lemon-smoothie.jpg",
    nutrition: { calories: 225.6, sodium: 19.5, sugar: 48.8, saturatedFat: 0.0, protein: 1.8, caffeine: null, allergens: null },
  },
  {
    id: "real-watermelon-juice",
    name: "Real Watermelon Juice",
    nameKo: "리얼 수박 주스",
    category: "blended",
    image: "/images/menu/real-watermelon-juice.jpg",
    nutrition: { calories: 142.1, sodium: 5.8, sugar: 26.2, saturatedFat: 0.0, protein: 1.3, caffeine: null, allergens: null },
  },
  {
    id: "red-apple-tea-smoothie",
    name: "Red Apple Tea Smoothie",
    nameKo: "홍사과 티 스무디",
    category: "blended",
    image: "/images/menu/red-apple-tea-smoothie-20260330.jpg",
    nutrition: null,
  },
  {
    id: "apple-smoothie",
    name: "Apple Smoothie",
    nameKo: "사과 스무디",
    category: "blended",
    image: "/images/menu/apple-smoothie-20260330.jpg",
    nutrition: null,
  },
  // Bakery
  {
    id: "coconut-levain-cookie",
    name: "Coconut Levain Cookie",
    nameKo: "코코넛 르뱅쿠키",
    category: "bakery",
    image: "/images/menu/coconut-levain-cookie.jpg",
    nutrition: { calories: 503, sodium: 152, sugar: 26, saturatedFat: 10, protein: 6, caffeine: null, allergens: null },
  },
  {
    id: "cranberry-levain-cookie",
    name: "Cranberry Levain Cookie",
    nameKo: "크랜베리 르뱅쿠키",
    category: "bakery",
    image: "/images/menu/cranberry-levain-cookie.jpg",
    nutrition: { calories: 483, sodium: 143, sugar: 31, saturatedFat: 4, protein: 5, caffeine: null, allergens: null },
  },
  {
    id: "matcha-levain-cookie",
    name: "Matcha Levain Cookie",
    nameKo: "말차 르뱅쿠키",
    category: "bakery",
    image: "/images/menu/matcha-levain-cookie.jpg",
    nutrition: { calories: 472, sodium: 152, sugar: 36, saturatedFat: 7, protein: 6, caffeine: null, allergens: null },
  },
  {
    id: "earl-grey-levain-cookie",
    name: "Earl Grey Levain Cookie",
    nameKo: "얼그레이 르뱅쿠키",
    category: "bakery",
    image: "/images/menu/earl-grey-levain-cookie.jpg",
    nutrition: { calories: 464, sodium: 124, sugar: 33, saturatedFat: 7, protein: 6, caffeine: null, allergens: null },
  },
  {
    id: "oreo-levain-cookie",
    name: "Oreo Levain Cookie",
    nameKo: "오레오 르뱅쿠키",
    category: "bakery",
    image: "/images/menu/oreo-levain-cookie.jpg",
    nutrition: { calories: 451, sodium: 143, sugar: 32, saturatedFat: 9, protein: 6, caffeine: null, allergens: null },
  },
  {
    id: "butter-rice-cake",
    name: "Butter Rice Cake",
    nameKo: "버터떡",
    category: "bakery",
    image: "/images/menu/butter-rice-cake.jpg",
    nutrition: { calories: 210, sodium: 176, sugar: 11, saturatedFat: 4.0, protein: 2, caffeine: null, allergens: null },
  },
  {
    id: "roasted-macadamia-financier",
    name: "Roasted Macadamia Financier",
    nameKo: "로스티드 마카다미아 휘낭시에",
    category: "bakery",
    image: "/images/menu/roasted-macadamia-financier.jpg",
    nutrition: { calories: 215, sodium: 107, sugar: 11, saturatedFat: 3, protein: 3, caffeine: null, allergens: null },
  },
  {
    id: "dubai-choco-financier",
    name: "Dubai Choco Financier",
    nameKo: "두바이 초코 휘낭시에",
    category: "bakery",
    image: "/images/menu/dubai-choco-financier.jpg",
    nutrition: { calories: 306, sodium: 111, sugar: 20, saturatedFat: 6, protein: 5, caffeine: null, allergens: null },
  },
  {
    id: "pearl-salt-financier",
    name: "Pearl Salt Financier",
    nameKo: "펄솔트 휘낭시에",
    category: "bakery",
    image: "/images/menu/pearl-salt-financier.jpg",
    nutrition: { calories: 159, sodium: 82, sugar: 12, saturatedFat: 3, protein: 2, caffeine: null, allergens: null },
  },
  {
    id: "honey-crispy-financier",
    name: "Honey Crispy Financier",
    nameKo: "허니 크리스피 휘낭시에",
    category: "bakery",
    image: "/images/menu/honey-crispy-financier.jpg",
    nutrition: { calories: 186, sodium: 66, sugar: 13, saturatedFat: 3, protein: 3, caffeine: null, allergens: null },
  },
  {
    id: "dubai-chewy-cookie",
    name: "Dubai Chewy Cookie",
    nameKo: "두바이 쫀득쿠키",
    category: "bakery",
    image: "/images/menu/dubai-chewy-cookie.jpg",
    nutrition: { calories: 258, sodium: 12.1, sugar: 9.1, saturatedFat: 10, protein: 2.3, caffeine: null, allergens: null },
  },
  {
    id: "canele",
    name: "Canele",
    nameKo: "까눌레",
    category: "bakery",
    image: "/images/menu/canele.jpg",
    nutrition: { calories: 288, sodium: 33, sugar: 19, saturatedFat: 1.0, protein: 5, caffeine: null, allergens: "밀/계란/견과류" },
  },
  {
    id: "ssuk-red-bean-smore-cookie",
    name: "Ssuk Red Bean Smore Cookie",
    nameKo: "쑥단팥 스모어쿠키",
    category: "bakery",
    image: "/images/menu/ssuk-red-bean-smore-cookie.jpg",
    nutrition: { calories: 571, sodium: 158, sugar: 38, saturatedFat: 10, protein: 8, caffeine: null, allergens: "밀/계란/견과류" },
  },
  {
    id: "valrhona-smore-cookie",
    name: "Valrhona Smore Cookie",
    nameKo: "발로나 스모어 쿠키",
    category: "bakery",
    image: "/images/menu/valrhona-smore-cookie.jpg",
    nutrition: { calories: 622, sodium: 116, sugar: 39, saturatedFat: 10, protein: 6, caffeine: null, allergens: "밀/계란/견과류" },
  },
  {
    id: "original-salt-bread",
    name: "Original Salt Bread",
    nameKo: "오리지널 소금빵",
    category: "bakery",
    image: "/images/menu/original-salt-bread.jpg",
    nutrition: { calories: 195, sodium: 272, sugar: 2, saturatedFat: 4, protein: 5, caffeine: null, allergens: "밀/계란/우유" },
  },
  {
    id: "egg-mayo-salt-bread-sandwich",
    name: "Egg Mayo Salt Bread Sandwich",
    nameKo: "에그마요 소금빵 샌드위치",
    category: "bakery",
    image: "/images/menu/egg-mayo-salt-bread-sandwich.jpg",
    nutrition: { calories: 490, sodium: 692, sugar: 2, saturatedFat: 13, protein: 2, caffeine: null, allergens: "밀/계란/우유/대두" },
  },
  {
    id: "ice-cream-salt-bread",
    name: "Ice Cream Salt Bread",
    nameKo: "아이스크림 소금빵",
    category: "bakery",
    image: "/images/menu/ice-cream-salt-bread.jpg",
    nutrition: { calories: 389.6, sodium: 378.1, sugar: 17.1, saturatedFat: 8.3, protein: 8.2, caffeine: null, allergens: "우유/대두/밀/계란" },
  },
  {
    id: "iced-milk-cream-salt-bread",
    name: "Iced Milk Cream Salt Bread",
    nameKo: "아이스 우유 크림 소금빵",
    category: "bakery",
    image: "/images/menu/iced-milk-cream-salt-bread.jpg",
    nutrition: { calories: 328, sodium: 330, sugar: 3, saturatedFat: 9, protein: 6, caffeine: null, allergens: "밀/계란/우유" },
  },
  {
    id: "raspberry-jam-scone",
    name: "Raspberry Jam Scone",
    nameKo: "라즈베리잼 스콘",
    category: "bakery",
    image: "/images/menu/raspberry-jam-scone.jpg",
    nutrition: { calories: 340, sodium: 190, sugar: 9, saturatedFat: 8, protein: 5, caffeine: null, allergens: "밀/우유/계란" },
  },
  {
    id: "milk-scone",
    name: "Milk Scone",
    nameKo: "밀크 스콘",
    category: "bakery",
    image: "/images/menu/milk-scone.jpg",
    nutrition: { calories: 359, sodium: 277, sugar: 12, saturatedFat: 9.0, protein: 5, caffeine: null, allergens: "밀/계란/견과류" },
  },
  {
    id: "almond-biscotti",
    name: "Almond Biscotti",
    nameKo: "아몬드 비스코티",
    category: "bakery",
    image: "/images/menu/almond-biscotti.jpg",
    nutrition: { calories: 77, sodium: 32, sugar: 3, saturatedFat: 1, protein: 2, caffeine: null, allergens: "밀/계란/견과류" },
  },
  {
    id: "chocochip-biscotti",
    name: "Chocochip Biscotti",
    nameKo: "초코칩 비스코티",
    category: "bakery",
    image: "/images/menu/chocochip-biscotti.jpg",
    nutrition: { calories: 76, sodium: 32, sugar: 5, saturatedFat: 2, protein: 1, caffeine: null, allergens: "밀/계란/견과류" },
  },
  {
    id: "derarita-original",
    name: "Derarita Original",
    nameKo: "데라리타 (오리지널)",
    category: "bakery",
    image: "/images/menu/derarita-original.jpg",
    nutrition: null,
  },
  {
    id: "derarita-sour-cream-onion",
    name: "Derarita Sour Cream & Onion",
    nameKo: "데라리타 (샤워크림&어니언)",
    category: "bakery",
    image: "/images/menu/derarita-sour-cream-onion.jpg",
    nutrition: null,
  },
  // Ice-cream
  {
    id: "soft-ice-cream",
    name: "Soft Ice Cream",
    nameKo: "소프트 아이스크림",
    category: "ice-cream",
    image: "/images/menu/soft-ice-cream.jpg",
    nutrition: { calories: 252.0, sodium: 56.0, sugar: 23.8, saturatedFat: 9.8, protein: 5.3, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "choco-ice-cream",
    name: "Choco Ice Cream",
    nameKo: "초코 아이스크림",
    category: "ice-cream",
    image: "/images/menu/choco-ice-cream.jpg",
    nutrition: { calories: 359.6, sodium: 132.5, sugar: 35.0, saturatedFat: 12.4, protein: 6.8, caffeine: 3.5, allergens: "우유/대두" },
  },
  {
    id: "apple-mango-ice-cream",
    name: "Apple Mango Ice Cream",
    nameKo: "애플망고 아이스크림",
    category: "ice-cream",
    image: "/images/menu/apple-mango-ice-cream.jpg",
    nutrition: { calories: 390.4, sodium: 124.4, sugar: 47.4, saturatedFat: 11.0, protein: 6.0, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "matcha-ice-cream",
    name: "Matcha Ice Cream",
    nameKo: "말차 아이스크림",
    category: "ice-cream",
    image: "/images/menu/matcha-ice-cream.jpg",
    nutrition: { calories: 786.4, sodium: 262.8, sugar: 68.4, saturatedFat: 24.4, protein: 13.3, caffeine: 93.2, allergens: "우유/대두" },
  },
  {
    id: "espresso-ice-cream",
    name: "Espresso Ice Cream",
    nameKo: "에스프레소 아이스크림",
    category: "ice-cream",
    image: "/images/menu/espresso-ice-cream.jpg",
    nutrition: { calories: 255.3, sodium: 64.0, sugar: 17.2, saturatedFat: 8.8, protein: 5.5, caffeine: 124.0, allergens: "우유/대두" },
  },
  {
    id: "strawberry-ice-cream",
    name: "Strawberry Ice Cream",
    nameKo: "딸기 아이스크림",
    category: "ice-cream",
    image: "/images/menu/strawberry-ice-cream.jpg",
    nutrition: { calories: 384.8, sodium: 111.4, sugar: 48.9, saturatedFat: 10.4, protein: 5.4, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "injeolmi-ice-cream",
    name: "Injeolmi Ice Cream",
    nameKo: "인절미 아이스크림",
    category: "ice-cream",
    image: "/images/menu/injeolmi-ice-cream.jpg",
    nutrition: { calories: 416.9, sodium: 138.7, sugar: 45.1, saturatedFat: 11.2, protein: 6.8, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "black-sugar-bubble-ice-cream-ic",
    name: "Black Sugar Bubble Ice Cream Ic",
    nameKo: "흑당 버블 아이스크림",
    category: "ice-cream",
    image: "/images/menu/black-sugar-bubble-ice-cream-ic.jpg",
    nutrition: { calories: 311.2, sodium: 76.2, sugar: 22.4, saturatedFat: 8.3, protein: 4.6, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "black-cookie-ice-cream",
    name: "Black Cookie Ice Cream",
    nameKo: "블랙쿠키 아이스크림",
    category: "ice-cream",
    image: "/images/menu/black-cookie-ice-cream.jpg",
    nutrition: { calories: 483.6, sodium: 262.9, sugar: 42.0, saturatedFat: 12.4, protein: 7.0, caffeine: null, allergens: "우유/대두" },
  },
  {
    id: "ice-cream-latte",
    name: "Ice Cream Latte",
    nameKo: "아이스크림 라떼",
    category: "ice-cream",
    image: "/images/menu/ice-cream-latte.jpg",
    nutrition: { calories: 362.7, sodium: 119.9, sugar: 42.2, saturatedFat: 10.2, protein: 7.5, caffeine: 147.0, allergens: "우유/대두" },
  },
  {
    id: "strawberry-cookie-ice-cream-frappe",
    name: "Strawberry Cookie Ice Cream Frappe",
    nameKo: "딸기 쿠키 아이스크림 프라페",
    category: "ice-cream",
    image: "/images/menu/strawberry-cookie-ice-cream-frappe.jpg",
    nutrition: { calories: 517.3, sodium: 276.2, sugar: 67.0, saturatedFat: 5.3, protein: 5.3, caffeine: null, allergens: "우유/대두/아황산류" },
  },
  {
    id: "matcha-cookie-ice-cream-frappe",
    name: "Matcha Cookie Ice Cream Frappe",
    nameKo: "말차 쿠키 아이스크림 프라페",
    category: "ice-cream",
    image: "/images/menu/matcha-cookie-ice-cream-frappe.jpg",
    nutrition: { calories: 559.9, sodium: 180.7, sugar: 78.6, saturatedFat: 5.3, protein: 6.7, caffeine: 53.7, allergens: "우유/대두" },
  },
  // MD
  {
    id: "md-togo-tumbler",
    name: "Blu Shaak To-Go Tumbler",
    nameKo: "블루샥 투고텀블러",
    category: "md",
    image: "/images/menu/md_1.jpg",
    nutrition: null,
  },
  {
    id: "md-coffee-cup",
    name: "Blu Shaak Coffee Cup",
    nameKo: "블루샥 커피컵",
    category: "md",
    image: "/images/menu/md_2.jpg",
    nutrition: null,
  },
  {
    id: "md-ballpoint-pen",
    name: "Blu Shaak Ballpoint Pen",
    nameKo: "블루샥 볼펜",
    category: "md",
    image: "/images/menu/md_3.jpg",
    nutrition: null,
  },
  {
    id: "md-tumbler",
    name: "Blu Shaak Tumbler",
    nameKo: "블루샥 텀블러",
    category: "md",
    image: "/images/menu/md_4.jpg",
    nutrition: null,
  },
  {
    id: "md-shaak-latte-metal-badge",
    name: "Shaak Latte Metal Badge",
    nameKo: "샥라떼 금속뱃지",
    category: "md",
    image: "/images/menu/md_5.jpg",
    nutrition: null,
  },
  {
    id: "md-symbol-metal-badge",
    name: "Blu Shaak Symbol Metal Badge",
    nameKo: "블루샥 심볼 금속뱃지",
    category: "md",
    image: "/images/menu/md_6.jpg",
    nutrition: null,
  },
  {
    id: "md-ice-tumbler-700ml",
    name: "Blu Shaak Ice Tumbler 700ml",
    nameKo: "블루샥 아이스텀블러 700ml",
    category: "md",
    image: "/images/menu/md_7.jpg",
    nutrition: null,
  },
  {
    id: "md-ice-tumbler-500ml",
    name: "Blu Shaak Ice Tumbler 500ml",
    nameKo: "블루샥 아이스텀블러 500ml",
    category: "md",
    image: "/images/menu/md_8.jpg",
    nutrition: null,
  },
  {
    id: "md-metro-mug-tumbler-475ml",
    name: "Blu Shaak Metro Mug Tumbler 475ml",
    nameKo: "블루샥 메트로 머그 텀블러 475ml",
    category: "md",
    image: "/images/menu/md_9.jpg",
    nutrition: null,
  },
  {
    id: "md-mug-cup",
    name: "Blu Shaak Mug Cup",
    nameKo: "블루샥 머그컵",
    category: "md",
    image: "/images/menu/md_10.jpg",
    nutrition: null,
  },
  {
    id: "md-capsule-coffee",
    name: "Blu Shaak Capsule Coffee",
    nameKo: "블루샥 캡슐커피",
    category: "md",
    image: "/images/menu/md_11.jpg",
    nutrition: null,
  },
  {
    id: "md-drip-bag",
    name: "Blu Shaak Drip Bag",
    nameKo: "블루샥 드립백",
    category: "md",
    image: "/images/menu/md_12.jpg",
    nutrition: null,
  },
  // Deli
  {
    id: "deli-chili-egg-plate",
    name: "Chili Egg Plate",
    nameKo: "칠리 에그 플레이트",
    category: "deli",
    image: "/images/menu/deli-placeholder.jpg",
    description: "웰니스 푸드인 '터키쉬 에그'를 블루샥만의 스타일로 풀어낸 요거트 에그 플레이트 / 요거트, 계란, 칠리오일, 양파 후레이크",
    nutrition: { calories: null, sodium: null, sugar: null, saturatedFat: null, protein: null, caffeine: null, allergens: "우유/계란/밀" },
  },
  {
    id: "deli-jambon-double-cheese-toast",
    name: "Jambon Double Cheese Toast",
    nameKo: "잠봉 더블치즈 토스트",
    category: "deli",
    image: "/images/menu/deli-placeholder.jpg",
    description: "호밀 사워도우에 짭짤한 체다치즈와 모짜렐라를 듬뿍 넣고, 풍미 가득한 잠봉을 더해 든든하고 따듯하게 즐기는 토스트 / 호밀 사워도우, 체다치즈, 모짜렐라 치즈, 잠봉",
    nutrition: { calories: null, sodium: null, sugar: null, saturatedFat: null, protein: null, caffeine: null, allergens: "밀/돼지고기/우유/대두/아황산류" },
  },
  {
    id: "deli-basil-chicken-cheese-toast",
    name: "Basil Chicken Cheese Toast",
    nameKo: "바질 치킨 치즈 토스트",
    category: "deli",
    image: "/images/menu/deli-placeholder.jpg",
    description: "호밀 사워도우에 향긋한 루꼴라, 신선한 토마토, 든든한 바질 치킨을 듬뿍 담아 만든 따듯한 토스트 / 호밀 사워도우, 루꼴라, 토마토, 바질 치킨",
    nutrition: { calories: null, sodium: null, sugar: null, saturatedFat: null, protein: null, caffeine: null, allergens: "밀/닭고기/우유/토마토/아황산류/대두/땅콩(캐슈넛)" },
  },
  {
    id: "deli-jambon-beurre-baguette",
    name: "Jambon Beurre Baguette",
    nameKo: "잠봉뵈르 바게트",
    category: "deli",
    image: "/images/menu/deli-placeholder.jpg",
    description: "겉은 바삭, 속은 촉촉한 바게트에 깔끔 담백한 버터와 풍미 가득한 잠봉이 조화로운 블루샥 스타일 잠봉뵈르 바게트 / 바게트, 버터, 잠봉",
    nutrition: { calories: null, sodium: null, sugar: null, saturatedFat: null, protein: null, caffeine: null, allergens: "밀/돼지고기/우유/아황산류" },
  },
  {
    id: "deli-basil-chicken-fresh-sandwich",
    name: "Basil Chicken Fresh Sandwich",
    nameKo: "바질 치킨 프레시 샌드위치",
    category: "deli",
    image: "/images/menu/deli-placeholder.jpg",
    description: "향긋한 루꼴라와 신선한 토마토, 든든한 바질 치킨과 상큼한 발사믹 글레이즈를 조화롭게 담아낸 블루샥 스타일 치킨 샌드위치 / 호밀 사워도우, 루꼴라, 토마토, 바질 치킨",
    nutrition: { calories: null, sodium: null, sugar: null, saturatedFat: null, protein: null, caffeine: null, allergens: "밀/닭고기/우유/토마토/아황산류/대두/땅콩(캐슈넛)" },
  },
  {
    id: "deli-jert",
    name: "JERT (Jambon·Egg·Rucola·Tomato)",
    nameKo: "제르트 (잠봉·에그·루꼴라·토마토)",
    category: "deli",
    image: "/images/menu/deli-placeholder.jpg",
    description: "풍미 가득한 잠봉(J), 든든한 계란(E), 향긋한 루꼴라(R), 신선한 토마토(T)를 든든하게 담아낸 블루샥만의 클래식 샌드위치 / 호밀 사워도우, 잠봉, 계란, 루꼴라, 토마토",
    nutrition: { calories: null, sodium: null, sugar: null, saturatedFat: null, protein: null, caffeine: null, allergens: "밀/돼지고기/우유/계란/토마토/대두/아황산류" },
  },
  {
    id: "deli-balsamic-burrata-salad",
    name: "Balsamic Burrata Salad",
    nameKo: "발사믹 부라타 샐러드",
    category: "deli",
    image: "/images/menu/deli-placeholder.jpg",
    description: "실키하고 고소한 부라타 치즈와 향긋한 루꼴라, 신선하고 아삭한 방울토마토를 상큼한 발사믹 글레이즈와 함께 조화롭게 담아내는 클래식 부라타 샐러드 / 부라타 치즈, 루꼴라, 방울토마토, 발사믹 글레이즈",
    nutrition: { calories: null, sodium: null, sugar: null, saturatedFat: null, protein: null, caffeine: null, allergens: "우유/토마토/아황산류" },
  },
  {
    id: "deli-basil-pesto-burrata-salad",
    name: "Basil Pesto Burrata Salad",
    nameKo: "바질 페스토 부라타 샐러드",
    category: "deli",
    image: "/images/menu/deli-placeholder.jpg",
    description: "실키하고 고소한 부라타 치즈와 향긋한 바질 페스토, 신선하고 아삭한 방울토마토가 조화로운 풍미 가득한 부라타 샐러드 / 부라타치즈, 방울토마토, 루꼴라, 바질페스토",
    nutrition: { calories: null, sodium: null, sugar: null, saturatedFat: null, protein: null, caffeine: null, allergens: "우유/토마토/땅콩(캐슈넛)" },
  },
];
